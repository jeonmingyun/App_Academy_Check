package com.example.academy_check_tablet.act;
...
import com.example.academy_check_tablet.Api.RetrofitApi;
import com.example.academy_check_tablet.R;
...
import static com.example.academy_check_tablet.Constants.Constants.BASE_URL;
...
public class TabletAct extends AppCompatActivity implements View.OnClickListener {
        ...
        switch (v.getId()) {
            ...
            case R.id.send_btn:
                ...
                if (student_code.length() == STUDENT_CODE_MAX_LENGTH) {
                    retrofitApiAction(student_code);
                } else {
    ...
    public void retrofitApiAction(String student_code) {
        try {
            ...
            api.timeCheck(student_code).enqueue(new Callback<String>() {
                @Override
            ...
            e.printStackTrace();
            Toast.makeText(this, "서버와 연결이 올바르지 않습니다.", Toast.LENGTH_SHORT).show();
        }
    }
}
